...
object CollectorRegistryPrometheus {
  ...
  def apply[F[_]: Sync](collectorRegistry: P.CollectorRegistry): CollectorRegistry[F] = {
    ...
    new CollectorRegistry[F] {
      ...
      def counter[A, B[_]](
        ...
        apply[A, B, P.Counter.Child, P.Counter, P.Counter.Builder, Counter[F]](
          counter,
          magnet.names(labels),
          List.empty,
        )
      ...
      def summary[A, B[_]](
        ...
        apply[A, B, P.Summary.Child, P.Summary, P.Summary.Builder, Summary[F]](
          summary,
          magnet.names(labels),
          List.empty,
        )
      ...
      def histogram[A, B[_]](
        ...
        apply[A, B, P.Histogram.Child, P.Histogram, P.Histogram.Builder, Histogram[F]](
          histogram,
          magnet.names(labels),
          List.empty,
        )
  ...
  private implicit def counterChildPrometheusToCounter[F[_]: Sync]: P.Counter.Child => Counter[F] =
    (a: P.Counter.Child) => {
      new Counter[F] {
        def inc(value: Double): F[Unit] = Sync[F].delay { a.inc(value) }
      }
  ...
  private implicit def summeryChildPrometheusToSummery[F[_]: Sync]: P.Summary.Child => Summary[F] =
    (a: P.Summary.Child) => {
      new Summary[F] {
        def observe(value: Double): F[Unit] = Sync[F].delay { a.observe(value) }
      }
  ...
  private implicit def histogramChildPrometheusToHistogram[F[_]: Sync]: P.Histogram.Child => Histogram[F] =
    (a: P.Histogram.Child) => {
      new Histogram[F] {
        def observe(value: Double): F[Unit] = Sync[F].delay { a.observe(value) }
      }
