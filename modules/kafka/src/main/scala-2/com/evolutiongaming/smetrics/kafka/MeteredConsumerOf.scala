...
class MeteredConsumerOf[F[_]](consumerOf: ConsumerOf[F],
                              kafkaMetricsRegistry: KafkaMetricsRegistry[F]) extends ConsumerOf[F] {

  override def apply[K, V](config: ConsumerConfig)(implicit
                                                   fromBytesK: FromBytes[F, K],
                                                   fromBytesV: FromBytes[F, V]): Resource[F, Consumer[F, K, V]] = {
    for {
...
object MeteredConsumerOf {
  def wrap[F[_]](consumerOf: ConsumerOf[F],
                 kafkaMetricsRegistry: KafkaMetricsRegistry[F]): MeteredConsumerOf[F] =
