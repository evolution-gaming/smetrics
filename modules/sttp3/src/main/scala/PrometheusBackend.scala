...
object SmetricsBackend {
  ...
  // trait LatencyMetric[F[_]] {
  //   type LN <: LabelNames
  //   type LV[Histogram[F]]
  //   def magnet1: LabelsMagnet[LN, LV]
  //   def labelValues: Request[_, _] => LV[Histogram[F]]
  //   def start: F[F[Unit]]
  // }

  // trait Mapper {
  //   def latency(req: Request[_, _]): List[String]
  //   def inProgress(req: Request[_, _]): List[String]
  //   def success(req: Request[_, _], resp: Response[_]): List[String]
  //   def error(req: Request[_, _], resp: Response[_]): List[String]
  //   def failure(req: Request[_, _], e: Throwable): List[String]
  //   def requestSize(req: Request[_, _], size: Long): List[String]
  //   def responseSize(req: Request[_, _], resp: Response[_]/** size: Long? */): List[String]
  // }
...
//    trait LatencyConfig[F[_]] {
//      type LN <: LabelNames
//      type LV[_]
//      type Metric = Request[_, _] => F[F[Unit]]
...
//              def stop: F[Unit]
//            }
//            def start(req: Request[_, _]): F[Stop]
//            def map0(f: Request[_, _] => LV[Histogram[F]]): F[Stop]
//          }
//        }
//        def resource(collectorRegistry: CollectorRegistry[F]): Resource[F, LV[Histogram[F]]]
//      }

//      def labelNames: LN
//      // def metric: LV[Histogram[F]]
//      // def labelValues(req: Request[_, _]): List[String]
//      def start(req: Request[_, _], metric: LV[Histogram[F]]): F[F[Unit]]
//      def meter: Resource[F, Metric]
//    }
    ...
    def of[F[_]: Monad](
      ...
      for {
        latencyHistogram <- collectorRegistry.histogram(
        ...
        def latency(req: Request[_, _]): F[F[Unit]]                      = Applicative[F].unit.pure[F]
        def inProgress(req: Request[_, _]): F[F[Unit]]                   = Applicative[F].unit.pure[F]
        def success(req: Request[_, _], resp: Response[_]): F[Unit]      = Applicative[F].unit
        def error(req: Request[_, _], resp: Response[_]): F[Unit]        = Applicative[F].unit
        def failure(req: Request[_, _], e: Throwable): F[Unit]           = Applicative[F].unit
        def requestSize(req: Request[_, _]): F[Unit]                     = Applicative[F].unit
...
object PrometheusBackend {
  ...
  // trait Mapper[F[_]] {
  //   def latency(req: Request[_, _]): Option[Histogram[F]]
  //   def inProgress(req: Request[_, _]): Option[CollectorConfig]
  //   def success(req: Request[_, _], resp: Response[_]): Option[CollectorConfig]
  //   def error(req: Request[_, _], resp: Response[_]): Option[CollectorConfig]
  //   def failure(req: Request[_, _], e: Throwable): Option[CollectorConfig]
  //   def requestSize(req: Request[_, _]): Option[CollectorConfig]
  //   def responseSize(req: Request[_, _], resp: Response[_]): Option[CollectorConfig]
  // }

  def apply[F[_], P](
      delegate: SttpBackend[F, P],
      requestToHistogramNameMapper: Request[_, _] => Option[Histogram[F]] = (req: Request[_, _]) =>
  ...
  /** Clear cached collectors (gauges and histograms) both from the given collector registry, and from the backend.
    */
  def clear(collectorRegistry: CollectorRegistry[_]): Unit = {
  ...
  private val histograms = new mutable.WeakHashMap[CollectorRegistry[_], ConcurrentHashMap[String, Histogram]]
  private val gauges     = new mutable.WeakHashMap[CollectorRegistry[_], ConcurrentHashMap[String, Gauge]]
  private val counters   = new mutable.WeakHashMap[CollectorRegistry[_], ConcurrentHashMap[String, Counter]]
  private val summaries  = new mutable.WeakHashMap[CollectorRegistry[_], ConcurrentHashMap[String, Summary]]

  private def cacheFor[T](
      cache: mutable.WeakHashMap[CollectorRegistry[_], ConcurrentHashMap[String, T]],
      collectorRegistry: CollectorRegistry[_]
